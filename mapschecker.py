...
def scan_gmaps(apikey):
    ...
    if response.text.find("errors") < 0:
        ...
        print("Reason: " +
              str(response.json()["error"]["errors"][0]["message"]))
    ...
    if response.status_code == 200:
        ...
        for lines in response.iter_lines():
            if (("TITLE") in str(lines)):
                print("Reason: " + str(lines).split("TITLE")[1].split("<")
                      [0].replace(">", ""))
...
if len(sys.argv) > 1:
    if sys.argv[1] == "--api-key" or sys.argv[1] == "-a":
        if len(sys.argv) > 2:
            ...
            print(
                "Either use --api-key as argument such \"python maps_api_scanner.py --api-key KEY\" or directly run script as \"python maps_api_scanner.py\" and supply API key via input."
            )
